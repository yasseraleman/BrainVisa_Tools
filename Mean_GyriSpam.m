function Mean_GyriSpam;
%pth = '/media/COSAS/Test/Joost'
pth = '/media/COSAS/Test/Joost/spanmesh';pths = pth;
%pths = '/media/COSAS/Test/Joost/parc/temp';
gtexfiles = sel_files(pth,'*_Lwhite_gyri_default_session_auto.tex');
spamfiles = sel_files(pth,'*_L_gyrvec.mesh');
Ns = size(gtexfiles,1);Ids = '';
for j =1:Ns
    %disp(['Computing Subject ']);
    [pth,nam,ext] = fileparts(deblank(spamfiles(j,:)));
    ind = strfind(nam,'-');
    ID = [nam(1:ind+10) 'white'];
    Ids = strvcat(Ids,ID);
    disp(['Computing Subject:  ==>  ' ID]);
    Sfile = [pths filesep ID '.mesh'];
    [Textg,Format] = read_texBrainvisa(deblank(gtexfiles(j,:)));
    SurfM = load_mesh(Sfile);
    Npoints = size(Textg.Values,1);
    SurfM.Is = Textg.Values(:,3);
    ind = find(SurfM.Is ==0);
    str = unique(SurfM.Is);
    Ns = max(str);SurfM.Is(ind) = Ns+1;
    [SurfM] = Surf_Corr(SurfM);
    ind = find(SurfM.Is ==Ns+1); SurfM.Is(ind) = 0;
    Surf = load_mesh(deblank(spamfiles(j,:)));
    ...
    d = sqrt(a+b+c);
    Surfd = SurfM;Surfd.Is = d;
    for i =1:size(str,1);
        if (str(i)~=0)|(str(i)~=0)|(str(i)~=0)
            ind = find(SurfM.Is==str(i));
            spammean(j,i) = mean(d(ind));
            spamcth(j,i) = std(d(ind));
        else
            spammean(j,i) = 0;
            spamcth(j,i) = 0;
        end
...
Mcads = createcads(spammean,charac);Mcads = [Ids Mcads];
Name = [pth filesep 'Results_MeanGyriSpan_Left.txt'];
save_resfiles(Name,Names,Mcads);
Name = [pth filesep 'Results_STDGyriSpan_Left.txt'];
Mcads = createcads(spamcth,charac);Mcads = [Ids Mcads];
save_resfiles(Name,Names,Mcads);
