function Multi_Replace_Sulc_name_Argfile(ChangeTxtFile,InputDir, hemi ,Outdir);
%
% Syntax :
% Multi_Replace_Sulc_name_Argfile(ChangeTxtFile,InputDir, hemi ,Outdir);
%
...
% Version $1.0
% 
ChangeTxtFile = '/media/COSAS/scripts/BrainVisa_myTools/Brainvisa_nomenclature_sulci+STS.txt';
InputDir = '/media/COSAS/8-BrainVISADataBase-HCP/subjects/100206/t1mri/default_acquisition/default_analysis/folds/3.1/default_session_auto/';
hemi = 'left';
Outdir = '/media/COSAS/8-BrainVISADataBase-HCP/subjects/100206/t1mri/default_acquisition/default_analysis/folds/3.1/default_session_auto/';
...
Args = sel_files(InputDir,[cad '*default_session_auto.arg']);
Cname = strvcat('size'); 
    ...
    for k=1:size(sts,1)
        if k==1
            ...
            if strcmp(Nsulcname(1:end-1),'unknown')
                ...
                Nsulcname = [Nsulcname(1:end-1) hemi];
            end
        else
            Sulclist = strvcat(Sulclist,[char(sts{k}) hemi]);
        end
    end
...
for i = 1:Ns
    ...
    OutArg = [Outdir filesep nm '_lobar.arg'];
    TempArg = [Outdir filesep nm '_temp.arg'];
    for j = 1:size(stsl,2)
        Temp = char(stsl{j});
        Sulclist = Temp(2:end,:);
        Nsulcname = deblank(Temp(1,:));
        OutArg = Replace_Sulc_name_Argfile(InArg,Sulclist,Nsulcname,OutArg);
    ...
    for k = 1:size(Cname,1)
        ...
        OutArg = Rename_sulci(TempArg,deblank(Cname(k,:)),Cth(k),OutArg,NLabelname);
        copyfile(OutArg,TempArg);
    end
    delete(TempArg);
end
